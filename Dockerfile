...
# copy scripts (layer clean and permissions fix)
# and make executable
# COPY --chmod=a+rwx resources/scripts/clean-layer.sh  /usr/bin/clean-layer.sh
# COPY --chmod=a+rwx resources/scripts/fix-permissions.sh  /usr/bin/fix-permissions.sh
...
RUN \
    ...
    libarchive-tools \
    # zlibc \  # obsolete in 22.04: https://unix.stackexchange.com/questions/667148/zlibc-package-not-found
...
# open resty
ENV OPEN_RESTY_VERSION="1.19.3.2"
RUN mkdir $RESOURCES_PATH"/openresty" && \
...
ENV CONDA_HOME="/opt/conda" \
    ...
    # MD5 is for miniconda version 4.9.2
    MINICONDA_MD5="122c8c9beb51e124ab32a0fa6426c656" \
    # SHA256 is for miniconda version 4.12.0
    MINICONDA_SHA256="3190da6626f86eee8abf1b2fd7a5af492994eb2667357ee4243975cdbb175d7a" \
...
RUN wget --no-verbose https://repo.anaconda.com/miniconda/Miniconda3-py38_${CONDA_VERSION}-Linux-x86_64.sh -O ~/miniconda.sh && \
    ...
    $CONDA_HOME/bin/conda config --system --set pip_interop_enabled false && \
    # Update conda
    $CONDA_HOME/bin/conda update -y -n base -c defaults conda && \
    $CONDA_HOME/bin/conda update -y setuptools && \
    $CONDA_HOME/bin/conda install -y conda-build && \
    # Update selected packages - install python 3.8.x
    $CONDA_HOME/bin/conda install -y --update-all python=$PYTHON_VERSION && \
    # Link Conda
    ln -s $CONDA_HOME/bin/python /usr/local/bin/python && \
    ...
    $CONDA_HOME/bin/conda clean -y --packages && \
    $CONDA_HOME/bin/conda clean -y -a -f  && \
    $CONDA_HOME/bin/conda build purge-all && \
    # Fix permissions
    fix-permissions.sh $CONDA_HOME && \
    clean-layer.sh
...
RUN apt-get update && \
    ...
    /usr/bin/npm install -g yarn typescript webpack node-gyp && \
    # # Install Yarn
...
RUN \
    # We hate the way xfce4 looks, so let's not use it
...
ENV LD_LIBRARY_PATH=/lib/x86_64-linux-gnu:/usr/lib/x86_64-linux-gnu:$CONDA_HOME/lib
...
# Holy fuck this next block is massive. Takes over an hour to complete,
# and there are lots of potential bugs/errors. Need to break into multiple
# RUN steps so that successful layers are cached and we don't need to 
# start from scratch for every debugging operation
#
# Plus there are a ton of different conda operations, which take forever 
# because conda has to re-solve the env for every conda call. Need to
# consolodate conda calls as much as possible.
# 
# Finally, can severely cut down on the number of Python packages that 
# get installed. It seems that only the "minimal" requirements are actually
# needed, since the full workspace is functional with just those. Many 
# of the others are entirely unnecessary for what we want to do, and we can
# always add things back if we need them at some point in the future.
...
RUN \
    ...
    # ln -s -f $CONDA_HOME/bin/python /usr/bin/python3 && \
    # if removed -> cannot use add-apt-repository
    # rm /usr/bin/python3 && \
    # rm /usr/bin/python3.5
    ln -s -f $CONDA_HOME/bin/python /usr/bin/python && \
    apt-get update && \
    # upgrade pip
    # pip install --upgrade pip && \
    # # for some reason, conda can't seem to install/build cysignals (it's a 
    # # dependency for at least one of the packages installed below)
    # # so we'll install it with pip prior to attempting the conda installs
    ...
    # needed to remove the --update-all flag, since it forced a reinstall 
    # of cysignals, which conda can't figure out how to do (installed with pip above)
    # conda install -y \
    ...
    pip install -r ${RESOURCES_PATH}/libraries/requirements-minimal.txt
...
# WTF? This also gets install in requirements-full, so 
...
RUN \
    ...
    # Fix permissions? fix-permissions.sh $CONDA_HOME && \
    # Cleanup
    clean-layer.sh
...
RUN \
    ...
    #
    # Remove all except minimal stuff
    #
    # if [ "$WORKSPACE_FLAVOR" = "minimal" ]; then \
